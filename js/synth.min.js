...
function init() {
    ...
    $(".property-name").mousedown(function() {
        if ($(this).not(".active")) {
            ...
            $(this).text() === "Distort Z" && (setting[pointTo] = "ruttEtraParams.originZ = pointer[i]");
    ...
    $(".close-button").on("click", function() {
        controls === !1 ? controls = !0 : controls = !1;
    });
    initComplete = !0;
    animate();
}
...
function meshChange() {
    ...
    switch (shape) {
      ...
      case "cube":
        ...
        break;
      case "cylinder":
        geometry = new THREE.CylinderGeometry(ruttEtraParams.scale, ruttEtraParams.scale, 240, 360, 240, !1);
        mesh = new THREE.Mesh(geometry, videoMaterial);
        break;
      case "torus":
...
function onToggleWebcam() {
    if (ruttEtraParams.webcam === !0) {
        ruttEtraParams.webcam = !0;
        videoInput.src = videoObject;
    } else {
        ruttEtraParams.webcam = !1;
...
f4.add(ruttEtraParams, "originX", -2e3, 2e3).step(1).listen().name("Distort X").onChange(onParamsChange);

f4.add(ruttEtraParams, "originY", -2e3, 2e3).step(1).listen().name("Distort Y").onChange(onParamsChange);

f4.add(ruttEtraParams, "originZ", -2e3, 2e3).step(1).listen().name("Distort Z").onChange(onParamsChange);

f4.add(ruttEtraParams, "opacity", 0, 1).step(.01).listen().name("Opacity").onChange(onParamsChange);

f4.add(ruttEtraParams, "hue", 0, 360).step(.1).name("Hue").onChange(onParamsChange);
