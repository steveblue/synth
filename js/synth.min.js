...
function newMesh(geo, scale) {
    scene.remove(mesh);
    geo === "plane" ? geometry = new THREE.PlaneGeometry(videoInput.videoWidth, videoInput.videoHeight, videoInput.videoWidth, videoInput.videoHeight) : geo === "sphere" ? geometry = new THREE.SphereGeometry(videoInput.videoHeight / 2, videoInput.videoHeight / 2, videoInput.videoHeight / 2) : geo === "cube" ? geometry = new THREE.CubeGeometry(videoInput.videoHeight / 3, videoInput.videoHeight / 3, videoInput.videoHeight / 3, videoInput.videoHeight / 3, videoInput.videoHeight / 3, videoInput.videoHeight / 3) : geo === "cylinder" ? geometry = new THREE.CylinderGeometry(scale * 2, scale * 2, videoInput.videoHeight / 2, videoInput.videoWidth / 2, videoInput.videoHeight / 2, !0) : geo === "torus" && (geometry = new THREE.TorusGeometry(scale * 2, videoInput.videoHeight / 2, videoInput.videoHeight / 2, videoInput.videoHeight / 2));
...
var container, camera, scene, renderer, video, texture, material, mesh, composer, renderModel, effectBloom, effectHue, effectCopy, mouseX = 0, mouseY = 0, windowHalfX = window.innerWidth / 2, windowHalfY = window.innerHeight / 2, videoInput = document.getElementById("video"), canvasInput = document.getElementById("compare"), initComplete = !1, count = 0, hex;
...
effectBloom = new THREE.BloomPass(2.4, 50, 18, 256);
...
f3.add(ruttEtraParams, "cameraz", -12e3, 12e3).step(1).listen().name("Zoom").onChange(onParamsChange);
