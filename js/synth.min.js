...
Synth.prototype = {
  get settings() {
    ...
    (shape === "plane" || shape === "cube" || shape === "torus" || shape === "sphere" || shape === "cylinder") && this.meshChange(shape)
  }, get color() {
    return this.hue
  }, set color(val) {
    this.hue = val
    ...
    this.camera = new THREE.PerspectiveCamera(40, window.innerWidth / window.innerHeight, 1, 1e4);
    ...
    this.fill = new THREE.AmbientLight(7368816);
    this.scene.add(that.fill);
    this.key = new THREE.SpotLight(16777215);
    this.key.position.set(0, 0, 5e3).normalize();
    this.key.target = this.mesh;
    this.key.intensity = 5e3;
    this.key.castShadow = !0;
    this.scene.add(that.key);
    this.back = new THREE.SpotLight(16777215);
    this.back.position.set(0, 0, -5e3).normalize();
    this.back.target = this.mesh;
    this.back.intensity = 5e3;
    this.back.castShadow = !0;
    this.scene.add(that.back);
    window.addEventListener("resize", onWindowResize, !1);
    this.control === !0 && this.initControls();
    this.cam === !0 && this.initWebcam();
    that.initComplete = !0;
    animate()
  },
  initWebcam: function() {
    ...
    if (Modernizr.getusermedia) {
      ...
      navigator.getUserMedia({
        ...
        if (navigator.mozGetUserMedia) that.videoInput.mozSrcObject = stream;
        else {
    ...
    } else $("header h2").text("Use your webcam and upload music and video in Google Chrome. Stay around and play with the controls in Safari.");
    that.webcam === !1 && that.playVideo(0)
  },
  ...
  initControls: function() {
    ...
    $(".xy").draggable({
      ...
      stop: function() {
        control = $(this).position();
        control.top < height && control.top > 0 && (value = that.convertToRange(control.top, [0, height], [start, end]));
        control.left < width && control.left > 0 && (value2 = that.convertToRange(control.left, [0, width], [start, end]));
        coords = that.convertTo3dCoords(value, value2, window.innerWidth, window.innerHeight, $(this).data("multiply"));
        eval("that." + key1 + "=" + coords.x + "");
        eval("that." + key2 + "=" + coords.y + "")
      }
    ...
    $(".bars li").on("click", function() {
      if (!$(this).hasClass("controller") && that.audioisplaying === !0) {
        that.trigger = !0;
      ...
      } else $(this).removeClass("controller")
    });
    $(".control").on("click", function() {
      if (that.trigger === !0 && !$(this).hasClass("controller")) {
    ...
    keypress.combo("m", function() {
      (that.menu = !0) ? that.menu(!1) : that.menu(!0)
    });
  ...
  playAudio: function(playlistId) {
    ...
    this.dancer.after(0, function() {
      that.audiostream[0] = Math.round(this.getFrequency(30) * 5e3);
      that.audiostream[1] = Math.round(this.getFrequency(60) * 5e3);
      that.audiostream[2] = Math.round(this.getFrequency(90) * 5e3);
      that.audiostream[3] = Math.round(this.getFrequency(120) * 5e3);
      that.audiostream[4] = Math.round(this.getFrequency(150) * 5e3);
      that.audiostream[5] = Math.round(this.getFrequency(180) * 5e3);
      that.audiostream[6] = Math.round(this.getFrequency(210) * 5e3);
      that.audiostream[7] = Math.round(this.getFrequency(240) * 5e3);
      that.audiostream[8] = Math.round(this.getFrequency(270) * 5e3);
      that.audiostream[9] = Math.round(this.getFrequency(300) * 5e3);
      that.audiostream[10] = Math.round(this.getFrequency(330) * 5e3)
  ...
  listResults: function(entries, type, context) {
    ...
    entries.forEach(function(entry, i) {
      var li = document.createElement("li"),
        name = unescape(entry.name),
        correctName = unescape(entry.name);
      correctName.length > 30 && (correctName = correctName.substring(0, 30));
        ...
        var dirReader = dirEntry.createReader(),
          ...
          readEntries = function() {
            dirReader.readEntries(function(results) {
              if (!results.length) that.listResults(entries.sort(), type, evt.target.context);
              else {
  ...
  paramsChange: function() {
    ...
    that.saturation >= -1 && that.saturation < 1 ? that.effectHue.uniforms.saturation.value = that.saturation : that.saturation < -1 ? that.effectHue.uniforms.saturation.value = -1 : that.saturation > 1 && (that.effectHue.uniforms.saturation.value = 1);
    $("#canvas").css("background-color", that.hex);
    var newhex = parseInt(that.hex.replace("#", "0x"));
    that.renderer.setClearColor(newhex, 1);
    if (that.audioisplaying === !0) {
  ...
  meshChange: function(shape, x, s) {
    var that = this;
    that.shape = shape;
    if (x === null || x === undefined) x = 64;
    if (s === null || s === undefined) s = 64;
    if (that.meshUpdate === !0) {
      that.scene.remove(that.mesh);
      that.geometry.verticesNeedUpdate = !1;
      that.geometry.dynamic = !1;
      that.geometry = null;
      that.videoMaterial.renderToScreen = !1
    }
    switch (shape) {
      case "plane":
      ...
      case "cube":
        ...
        break;
      case "cylinder":
        that.geometry = new THREE.CylinderGeometry(that.scale * 2, that.scale * 2, x * 8, s, x, !1);
        that.mesh = new THREE.Mesh(that.geometry, that.videoMaterial);
        break;
      case "torus":
    ...
    setTimeout(function() {
      ...
      that.videoMaterial.renderToScreen = !0
    }, 100)
  },
